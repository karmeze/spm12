...
%Modified 2015-08 @LIBD by: Karleigh Mezeivtch

%Print Brain section in table
...
%==========================================================================
    ...
    TabDat.hdr = {...
        'set',      'p',            '\it p';...
        'set',      'c',            '\itc';...
        'cluster',  'p(FWE-corr)',  '\it p\rm_{FWE}';...
        'cluster',  'p(FDR-corr)',  '\it q\rm_{FDR}';...
        'cluster',  'equivk',       '\itk\rm_E';...
        'cluster',  'p(unc)',       '\it p\rm_{uncorr}';...
        'peak',     'p(FWE-corr)',  '\it p\rm_{FWE}';...
        'peak',     'p(FDR-corr)',  '\it q\rm_{FDR}';...
        'peak',      STAT,          sprintf('\\it%c',STAT);...
        'peak',     'equivZ',       '(\itZ\rm_\equiv)';...
        'peak',     'p(unc)',       '\it p\rm_{uncorr}';...
        '',         'x,y,z {mm}',   [units{:}];...
   ...
   %KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK
    ...
    zz=1;
    for k=1:m(1)
        pos=TabDat.dat{zz, 12};
    
   
        [des]= gcap_displayMni2(pos,voxsz);
        
        cc=1;
        
        for c= 1:length(des)
    
            var.des{zz,cc}= [des{1,c}];
            cc= cc+1;
    ...
    %----------------------------------------------------------------------
    % set(hAx,'DefaultTextFontName',PF.helvetica,'DefaultTextFontSize',FS(7))
    set(hAx,'DefaultTextFontName','Arial','DefaultTextFontSize',FS(7))
    %KKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKKK
    Hs = []; Hc = []; Hp = [];
    h  = text(0.01,y, [TabDat.hdr{1,1} '-level'],'FontSize',FS(8)); Hs = [Hs,h];
    h  = line([0,0.09],[1,1]*(y-dy/4),'LineWidth',0.5,'Color','r'); Hs = [Hs,h];
    h  = text(0.01,y-9*dy/8,    TabDat.hdr{3,1},'FontSize',FS(8));              Hs = [Hs,h];
    h  = text(0.07,y-9*dy/8,    TabDat.hdr{3,2},'FontSize',FS(8));              Hs = [Hs,h];
    
    h = text(0.17,y, [TabDat.hdr{1,3} '-level'],'FontSize',FS(8));    Hc = [Hc,h];
    h = line([0.11,0.36],[1,1]*(y-dy/4),'LineWidth',0.5,'Color','r'); Hc = [Hc,h];
    h  = text(0.11,y-9*dy/8,    TabDat.hdr{3,3});              Hc = [Hc,h];
    h  = text(0.17,y-9*dy/8,    TabDat.hdr{3,4});              Hc = [Hc,h];
    h  = text(0.25,y-9*dy/8,    TabDat.hdr{3,5});              Hc = [Hc,h];
    h  = text(0.30,y-9*dy/8,    TabDat.hdr{3,6});              Hc = [Hc,h];
    
    h = text(0.51,y, [TabDat.hdr{1,7} '-level'],'FontSize',FS(8));    Hp = [Hp,h];
    h = line([0.39,0.71],[1,1]*(y-dy/4),'LineWidth',0.5,'Color','r'); Hp = [Hp,h];
    h  = text(0.39,y-9*dy/8,    TabDat.hdr{3,7});              Hp = [Hp,h];
    h  = text(0.455,y-9*dy/8,    TabDat.hdr{3,8});              Hp = [Hp,h];
    h  = text(0.56,y-9*dy/8,    TabDat.hdr{3,9});              Hp = [Hp,h];
    h  = text(0.59,y-9*dy/8,    TabDat.hdr{3,10});             Hp = [Hp,h];
    h  = text(0.645,y-9*dy/8,    TabDat.hdr{3,11});             Hp = [Hp,h];
    
    text(0.73,y - dy/2,TabDat.hdr{3,12},'Fontsize',FS(8));
  
    text(0.855,y - dy/2,TabDat.hdr{3,13},'Fontsize',FS(8));
    
    
    %-Move to next vertical position marker
    %----------------------------------------------------------------------
    y     = y - 7*dy/3;
    line([0 1],[y y],'LineWidth',0.7,'Color','r')
    ...
    %----------------------------------------------------------------------
    tCol = [ 0.01      0.07 ...                                %-Set
             0.11      0.18      0.245      0.305 ...            %-Cluster
             0.39      0.46      0.52      0.58      0.645 ...  %-Peak
             0.715...                                           %-XYZ
             0.835];                                            %-Section
   %MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM 
    ...
    for i=1:size(TabDat.dat,1)
        ...
        for k=[(3:11),13]
            h = text(tCol(k),y,sprintf(TabDat.fmt{k},TabDat.dat{i,k}),...
                     ...
                     'ButtonDownFcn','get(gcbo,''UserData'')');
            if k==13
                set(h, 'FontSize', FS(6), 'FontName', 'Arial');
            end
            hPage = [hPage, h];
    ...
    % FORMAT spm_list('TxtList',TabDat,c)
        ...
        for i=1:max(1,13-size(TabDat.dat,1)), fprintf('\n'), end
        fprintf('%s\n',TabDat.str)
        fprintf('%c',repmat('-',1,80)), fprintf('\n')
